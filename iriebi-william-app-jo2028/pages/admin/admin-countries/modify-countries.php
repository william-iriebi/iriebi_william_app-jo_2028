...
try {
    $queryCountry = "SELECT nom_pays FROM PAYS WHERE id_pays = :idPays";
    $statementCountry = $connexion->prepare($queryCountry);
    $statementCountry->bindParam(":idPays", $id_pays, PDO::PARAM_INT);
    $statementCountry->execute();

    if ($statementCountry->rowCount() > 0) {
        $pays = $statementCountry->fetch(PDO::FETCH_ASSOC);
    } else {
...
<body>
    ...
    <main>
        ...
        <form action="modify-countries.php?id_pays=<?php echo $id_pays; ?>" method="post" onsubmit="return confirm('Êtes-vous sûr de vouloir modifier ce pays?')">
