...
<body>
    ...
    <main>
        <h1>Résultats des Jeux Olympiques 2028</h1>
        ...
        try {
            ...
            $query = "SELECT s.nom_sport AS sport, e.nom_epreuve AS event, CONCAT(a.prenom_athlete, ' ', a.nom_athlete) AS winner, e.date_epreuve AS date
                      FROM PARTICIPER p
            ...
            if ($statement->rowCount() > 0) {
                echo "<table>";
                echo "<tr><th class='color'>Sport</th><th class='color'>Événement</th><th class='color'>Gagnant</th><th class='color'>Date</th></tr>";

                // Afficher les données dans un tableau
                while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($row['sport'], ENT_QUOTES, 'UTF-8') . "</td>";
                    echo "<td>" . htmlspecialchars($row['event'], ENT_QUOTES, 'UTF-8') . "</td>";
                    echo "<td>" . htmlspecialchars($row['winner'], ENT_QUOTES, 'UTF-8') . "</td>";
